...
from .....domain import DDD_NAMING_REGISTRY, DirectionEnum
...
def get_naming_cloud_html() -> str:
    ...
    TYPE_HEADER_STYLE = (
        "font-weight: bold; color: #555; margin-top: 4px; font-size: 11px;"
    )
    ...
    for layer, directions in DDD_NAMING_REGISTRY.items():
        for direction, types in directions.items():
            ...
            for comp_type, regex_list in types.items():
                # Extract clean type name from Enum
                type_name = str(comp_type).split(".")[-1]
    ...
    for group_key in display_order:
        ...
        for type_name, tokens in sorted(items):
            ...
            tags_str = " ".join(
                [f'<span style="{TAG_STYLE}">{t}</span>' for t in tokens]
            )

            html_parts.append('<div style="margin-left: 2px; margin-bottom: 2px;">')
            html_parts.append(
                f'<span style="{TYPE_HEADER_STYLE}">{type_name}:</span> {tags_str}'
            )
